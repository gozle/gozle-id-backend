...
class AuthTest(APITestCase):
    ...
    def test_sign_up_success(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn("access_token", response.data.json())
        self.assertIn("refresh_token", response.data.json())

        refresh_token = response.data.json()['refresh_token']

        # Test Get User
        response = self.client.get('/api/auth/get-user',
                                   headers={"Authorization": f"Bearer {response.data.json()['access_token']}"})
        self.assertEqual(response.status_code, 200)
